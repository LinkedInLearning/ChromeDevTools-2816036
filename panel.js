...
chrome.runtime.onConnect.addListener((port) => {
  ...
  function addTagToContent(card, comment, baseUrl, boardId, accessToken) {
    if(comment && comment.text) {
      ...
      chrome.tabs.query({currentWindow: true, active: true}, (tabs) => {
        const commentBody = "gloCommentTag=" + tabs[0].url + "?posX=10&posY=10 gloCommentTagText=";
        postData(url, commentBody)
          .then((commentResp) => {
            sendMessage({subject: "renderComment", resp: {card, comment: {commentId: commentResp.id, posX: "10", posY: "10", commentText: {text: ""}}}});
          })
      })
    }
  }
